...
def process_video(video_name, video_url):
	...
	try:
		...
		return {'description':'Failed to Decode json.'}
		...
		for File in UnprocessedFiles:
			...
			try:
				...
				if 'Connection Aborted' in e:
					print('  UploadHost Error: Connection Aborted')
				else:
					print('  UploadHost Error',e)
				'''
...
if __name__ == '__main__':
	...
	for CameraDir in glob(PATH_TO_CAMERAS+'Camera*\\'):
		...
		mp.Process(target=UploadHost, args=(Camera,)).start()
		#break
		
		
		'''
		#$ Use asyncio instead of multiprocessing requests
